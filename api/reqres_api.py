...
class ReqresApi(Api):
    ...
    def reqres_update(self, user_id: int, param_request_body: RequestUpdateUserModel):
        return self.put(url=self._URL, endpoint=self._ENDPOINT + str(user_id), json_body=param_request_body.to_dict())

    @allure.step("Обращение к delete")
    def reqres_delete(self, user_id: int):
        return self.delete(url=self._URL, endpoint=self._ENDPOINT + str(user_id))
